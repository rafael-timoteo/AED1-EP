...
Boolean insere_ord_rec_linhas(No_arvore_linhas * raiz, No_arvore_linhas * novo, int * p_valCharLinhaRef){
	...
	if(novo->valChar != raiz->valChar){
		...
		novo->valChar++;
		*p_valCharLinhaRef = novo->valChar;
		return insere_ord_rec_linhas(raiz, novo, p_valCharLinhaRef);
	}
...
Boolean insere_ord_palavras(Arvore_palavras * arvore, Elemento e, float p_valChar, int p_linha, int p_valCharLinhaRef){
	...
	if( (no = busca_bin_palavras_aux(arvore, p_valChar)) ){
		...
		if(strcmp(aux1, aux2) == 0){
			...
			nova_linha->prox = no->linhas->primeiro;
			no->linhas->primeiro = nova_linha;
			return TRUE;
		}		
		else{
			free(aux1);
			...
			if( (no_busca_colisao = busca_colisao(no->colisao, e)) ){
				...
				nova_linha_colisao->prox = no_busca_colisao->linhas->primeiro;
				no_busca_colisao->linhas->primeiro = nova_linha_colisao;
				return TRUE;
			}
			else{
...
Boolean imprime_linha_bin_rec(No_arvore_linhas * raiz, int p_valChar){
	if(raiz){
		if(raiz->valChar == p_valChar){
			...
			if(p_valChar < raiz->valChar)
				return imprime_linha_bin_rec(raiz->esq, p_valChar);
			else
				return imprime_linha_bin_rec(raiz->dir, p_valChar);
		}
	}
	return FALSE;
}

Boolean imprime_linha_bin(Arvore_linhas * arvore, int p_valChar){
	No_arvore_linhas * p = arvore->raiz;
	if(p)
		return imprime_linha_bin_rec(p, p_valChar);
	return FALSE;
}
