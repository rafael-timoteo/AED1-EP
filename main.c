...
/**   Turma 04 - Prof. Flávio Luiz Coutinho                         **/
/**                                                                 **/
/**   Exercicio-Programa                                            **/
/**                                                                 **/
/**	  Nomes:                                      N°s USP:          **/
...
/**	  <Guilherme Augusto Arrabal de Souza>        <5642180>Turma 94 **/
/**                                                                 **/
/*********************************************************************/
	...
	if(in){
		...
		if(argc == 3){
			...
			while(in && fgets(linha, TAMANHO, in)){
				...
				//imprime_seq(a, contador_linha + 1);
				...
				// o conteúdo da linha anterior é sobreescrito.
					...
					//printf("Inserindo palavra '%s' na lista...", palavra);
					...
					//printf("%d caracteres na palavra %s\n", quant_char, ponteiro);
			...
			//Implementação do código com estrutura de dados do tipo árvore
				...
				contador_linha = 0;
 				...
 				while(in && fgets(linha, TAMANHO, in)){
					...
					while( (palavra = strsep(&copia_ponteiro_linha, " ")) ){
						...
						free(palavra_aux);				
						//printf("soma_val_char: %f\n", soma_val_char_palavra);
				...
				printf("Numero de linhas no arquivo: %d\n", contador_linha);
				//imprime_linhas(arvore_linhas);	
				//imprime_palavras(arvore_palavras);
					...
					if(strcmp(busca, "busca") == 0){
						busca = strsep(&copia_ponteiro_teste, " ");
							...
							for(aux_linhas_ = p->linhas->primeiro;aux_linhas_; aux_linhas_ = aux_linhas_->prox){
								imprime_linha_bin(arvore_linhas, aux_linhas_->valCharLinha);
							}
							t = clock() - t;
							printf("Tempo de busca: %lf ms\n", ((double)t));
						}
